...
def Cauchy(x, alpha, beta):
    ...
    :return: (float) the value of the Cauchy distribution at x
    """
    return beta / (beta**2 + (x - alpha) ** 2) / np.pi


def log_normal(I, alpha, beta, I_0, sigma):
    d = np.sqrt(beta**2 + (alpha - I)**2)
    mu = np.log(I_0/(d**2))
    return 1/np.sqrt(2*np.pi*sigma**2) * np.exp(-1/2 * (np.log(I) - mu)**2 / sigma**2)


def uniform(alpha, beta, **kwarg):
    """
    Prior distribution
    ...
    :param beta_max: float

    :return: (float) the value of the prior distribution at (alpha, beta)
    """
    if alpha < kwarg['alpha_min'] or alpha > kwarg['alpha_max'] or beta < kwarg['beta_min'] or beta > kwarg['beta_max']:
        return 0
    else:
        return 1 / (kwarg['alpha_max'] - kwarg['alpha_min']) / (kwarg['beta_max'] - kwarg['beta_min'])
...
class MetropolisHastings:
    def __init__(self, 
            target_func = None, # the target function
